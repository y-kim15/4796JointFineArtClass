...
def custom_resnet(add_reg=None, alpha=None, init=None, n=0,dp_rate=0):
    ...
    split_value = len(model.layers) + 1 - int(n)
...
def resnet_dropout(regularizer=False,include_top=False, weights='imagenet', input_tensor = None, pooling='avg', input_shape=(224,224,3),classes=25,dp_rate=0.,n_retrain_layers=0):
    ...
    split_value = len(model.layers) + 1 - int(n_retrain_layers)
    for layer in model.layers[:split_value]:
        layer.trainable = False
    for layer in model.layers[split_value:]:
        layer.trainable = True
